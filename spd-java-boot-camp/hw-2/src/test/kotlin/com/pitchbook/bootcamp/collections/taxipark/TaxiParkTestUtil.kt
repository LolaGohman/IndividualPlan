...
fun taxiPark(drivers: Set<Driver>, passengers: Set<Passenger>, vararg trips: Trip) =
        TaxiPark(drivers, passengers, trips.toList())
...
fun trip(driverIndex: Int, passenger: Int) =
        Trip(driver(driverIndex), passengers(passenger), 10, 3.0, null)
fun trip(driverIndex: Int, passengers: List<Int>) =
        Trip(driver(driverIndex), passengers(passengers), 10, 3.0, null)
...
fun TaxiPark.display() = buildString {
    ...
    appendln("Trips: ${allTrips.map { it.display() }}")
}
